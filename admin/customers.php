...
if (isset($_POST['edit_customer'])) {
    ...
    $stmt = $conn->prepare("UPDATE customers SET full_name=?, phone_number=?, address=? WHERE id=?");
    $stmt->bind_param("sssi", $full_name, $phone_number, $address, $id);
    $stmt->execute();
    $stmt->close();
    header("Location: customers.php");
    exit();
                        ...
                        <tbody>
                            <?php if ($result && $result->num_rows > 0): ?>
                                <?php while ($row = $result->fetch_assoc()): ?>
                                    <tr>
                                        <td><?= $row['id'] ?></td>
                                        <td class="fw-bold text-start"><?= htmlspecialchars($row['full_name']) ?></td>
                                        <td><?= htmlspecialchars($row['phone_number']) ?></td>
                                        <td><?= htmlspecialchars($row['address']) ?></td>
                                        <td>
                                            <button class="btn btn-sm btn-warning" data-bs-toggle="modal" data-bs-target="#editCustomer<?= $row['id'] ?>"><i class="fas fa-edit"></i> Sửa</button>
                                            <a href="customers.php?delete_id=<?= $row['id'] ?>" class="btn btn-sm btn-danger" onclick="return confirm('Xóa khách hàng này?');"><i class="fas fa-trash"></i> Xóa</a>
                                        </td>
                                    </tr>

                                    <div class="modal fade" id="editCustomer<?= $row['id'] ?>" tabindex="-1">
                                        <div class="modal-dialog">
                                            <div class="modal-content text-start">
                                                <div class="modal-header bg-warning text-dark">
                                                    <h5 class="modal-title fw-bold">Sửa Khách Hàng</h5>
                                                ...
                                                <form action="customers.php" method="POST">
                                                    ...
                                                    <div class="modal-footer">
                                                        ...
                                                        <button type="submit" name="edit_customer" class="btn btn-warning fw-bold">Cập nhật</button>
                                                    </div>
                                                </form>
                                            </div>
                                        </div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="customers.php" method="POST">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Hủy</button>
                    <button type="submit" name="add_customer" class="btn btn-success fw-bold">Lưu lại</button>
                </div>
            </form>
        </div>
    </div>
